...
\subsubsection{Filtering non-essential tweets}
We first look at tweets classified as \textbf{time\_critical} by the CrisisBench dataset but predicted as \textbf{non\_informative} by DeBERTa. After manually inspecting all 358 such tweets, we found that nearly all were incorrectly labeled as time\_critical in the original dataset and did not represent actionable, urgent, or location-specific cries for help. Many were entirely unrelated to crises, such as the tweet, ``how many fertilizer plants are there in texas?...'', which provides no emergency context. Others referenced crisis-related events but lacked any actionable content, such as ``...\#HurricaneODILE so many crashes and crumbles. Water coming in our 5th floor hotel room. Terrifying.'' While this tweet reflects a hazardous situation, it contains no request for assistance, location details, or actionable information. Out of the entire set, only a single tweet appeared potentially actionable: ``...Update -- Has Dementia! pls find him \#Missing ...``, though even this example is ambiguous, as it is unclear whether it relates to an ongoing crisis or a missing-person case independent of a disaster.
Overall, these findings demonstrate that DeBERTa is highly effective at filtering out non-actionable content even when the ground truth labels incorrectly mark such tweets as \textbf{time\_critical}. This ability is extremely valuable in real-time disaster triage, where excessive false alarms can overwhelm emergency systems and hinder the prioritization of genuine calls for rescue.
\subsubsection{Escalating non-essential tweets}
Since we examined tweets labeled as \textbf{time\_critical} but predicted as \textbf{non\_informative}, we now turn to the opposite case: tweets whose true label was \textbf{non\_informative} but were predicted as \textbf{time\_critical}. Although many of these tweets reference crises such as hurricanes or wildfires, DeBERTa often struggles to distinguish between actionable and non-actionable content. A common failure mode involves over-sensitivity to words associated with harm or danger. For example, the tweet ``Tattingstone suitcase murder: Police appeal over Bernard Oliver death...`` contains the word ``death'' but is entirely unrelated to an ongoing crisis, yet DeBERTa incorrectly flagged it as time\_critical. Similarly, the tweet ``At least 21 dead. Over 1 million forced into shelters. Typhoon \#Hagupit is yet...'' is tied to a real crisis but does not present any \textbf{actionable information}, such as a request for rescue or instructions for evacuation. DeBERTa nevertheless labeled it as time\_critical solely because it describes fatalities and large-scale impact.

However, despite this tendency to over-predict urgency, there was also one case where DeBERTa correctly identified a genuinely critical situation that had been mislabeled as non\_informative in the dataset: ``RT @anubhavsinha: Need Help for Ram, ... Stuck 5 hours west of Kathmandu w/broken left hand and leg. (cont)''. This tweet clearly expresses an urgent need for assistance, yet the original label failed to capture its time-sensitive nature. In this instance, DeBERTa appropriately elevated the tweet to \textbf{time\_critical}.
Overall, these observations suggest that while DeBERTa frequently overestimates urgency for crisis-related but non-actionable tweets, particularly those containing words like ``dead'' or ``death'', it can also provide meaningful corrections by recognizing mislabeled yet genuinely urgent cries for help.

\subsubsection{Mislabeling hashtag-heavy tweets}
Tweets labeled as \textbf{support\_and\_relief} in the CrisisBench dataset frequently rely on hashtags to express meaning, or ask for assistance. This is expected, as hashtags often act as the primary mechanism for community coordination during crises. However, because our preprocessing pipeline removed hashtags entirely, many such tweets lost their core semantic content. For example, the tweet ``\#NepalEarthquake Looking for \#CamilaYEzequiel \#Please \#RT'' becomes only ``looking for'' after preprocessing and is too vague for DeBERTa to recognize as support\_and\_relief, causing it to be classified as \textbf{non\_informative}. On the contrary, messages that do not rely heavily on hashtags for context, such as ``Lots of backyard work can be time consuming, but linemen do \#WhateverItTakes to get the lights back on! \#Irma\ldots'', still maintain its original meaning and is DeBerta correctly classifies this as \textbf{non\_informative} even though it was incorrectly labeled as \textbf{support\_and\_relief} in the original dataset. This suggests that some misclassifications originate from the dataset labels rather than the model. These findings indicate that many errors could possibly be attributed to preprocessing decisions, particularly the removal of hashtags, rather than limitations of DeBERTa itself.

\subsubsection{Overall Insights}
Overall, DeBerta excels at filtering out non-actionable content, which is highly beneficial in real disaster-response scenarios where minimizing false alarms is essential for effective triage. However, we observed several weaknesses. The model tends to overreact to specific keywords such as “help,” “dead,” “death,” or “explosion”, even when they appear in non-emergency or unrelated contexts. It also struggles with tweets in the \textbf{support\_and\_relief} category because much of their meaning is encoded in hashtags that lose semantic value during preprocessing. To improve performance, future iterations should consider preserving crisis-related hashtags so models can better capture the contextual signals needed for accurate classification.
