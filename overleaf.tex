...
In the event of a disaster, social media platforms like \textbf{Twitter (X)} serve as a critical channel for real-time information. However, these platforms suffer from a \textbf{flood of information} where useful intelligence for rescue and survivor support is mixed with \textbf{irrelevant or non-actionable content}. This mixture complicates on-the-ground response efforts.

The objective of this research is to address this challenge by developing a deep learning based classification system designed to \textbf{automatically filter and classify meaningful information} within disaster-related tweets. Specifically, we redefine the classification task by selecting \textbf{three core labels} from the 11 original humanitarian categories in the CrisisBench dataset~\cite{CrisisBenchPaper} that are most essential for immediate disaster response. We then compare the performance of three distinct DL architectures (TextCNN with GloVe, a custom transformer, and a pretrained transformer) to determine which model is most effective for this task.
