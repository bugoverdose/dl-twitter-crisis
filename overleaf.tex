...
\textbf{Explain learnable parameters and hyperparameters, and then explain which hyperparameters were chosen using validation curve}

\subsection{CNN}

% TBA: add number of parameters to be trained?

\textbf{Architecture} - only use the hyperparameter information
...
Best configuration: learning rate = $3 \times 10^{-4}$, 
...
optimizer = AdamW (learning rate = $3 \times 10^{-4}$, weight decay = 0.01)
...
Best configuration: learning rate = $5 \times 10^{-6}$, batch size = 16, weight decay = 0.01, epoch=7
...
\begin{table*}
    ...
    \begin{tabular}{|p{12cm}|c|}
        ...
        \hline
        how many fertilizer plants are there in texas? http://t.co/es4El50Hdx & Not crisis related \\
        \hline
        RT @severestudios: RT @MattMilletto: \#HurricaneODILE so many crashes and crumbles. Water coming in our 5th floor hotel room. Terrifying. & Crisis related but not actionable \\
        \hline
        @achohelelwhy: Update - Has Dementia! pls find him \#Missing \#Joplin Charles Writer (http://on.fb.me/kHaGEY) patient at \#Greenbrier ... & Actionable but unclear \\
        \hline
    \end{tabular}
    \end{center}
    \caption{Examples of true label time\_critical, predicted label non\_informative}
    \label{tab:qualitative_examples_tc_ni}
\end{table*}

\subsubsection{non\_informative vs time\_critical (551 Total)}
Since we looked at tweets labeled as time\_critical but predicted as non\_informative, we now look at the opposite case: tweets where the true label was non\_informative, but were predicted as time\_critical. Although overall the tweets are related to crises such as hurricanes, wildfires, etc., it struggles with distinguishing between actionable and non-actionable items. Also, as can be seen in Table~\ref{tab:qualitative_examples_ni_tc}, DeBerta struggles with time\_critical false positives where it is over-sensitive to tweets containing words such as “dead, death, deadly” and label them as time\_critical even though they are not crisis-related. Although in general DeBerta shows a tendency to overly classify tweets as time\_critical, there was 1 tweet that was actually \textbf{time\_critical} but was mis-labeled as \textbf{non\_informative} (see Table~\ref{tab:qualitative_examples_ni_tc}. The last tweet in the table is a clear cry for help, and DeBerta correctly classified the tweet as \textbf{time\_critical}. Therefore we can conclude that DeBERTa often over-predicts the \textbf{time\_critical} label for crisis-related but non-actionable tweets, particularly those containing words like “dead” or “death”, yet also demonstrates value by correctly elevating at least one mislabeled but genuinely urgent cry for help.
...
\begin{table*}
    ...
    \begin{tabular}{|p{12cm}|c|}
        ...
        \textbf{Tweet} & \textbf{Category} \\
        \hline
        Tattingstone suitcase murder: Police appeal over Bernard Oliver death - BBC News \url{https://t.co/epTuBCczG6} & Not crisis related \\
        \hline
        At least 21 dead. Over 1 million forced into shelters. Typhoon \#Hagupit is yet another reminder that we have no... \url{http://t.co/je8fwbV8jr} & Crisis related but not actionable \\
        \hline
        RT @anubhavsinha: Need Help for Ram, my House Help for 15 years. Stuck 5 hours west of Kathmandu w/broken left hand and leg. (cont) & Critical and actionable \\
        \hline
    \end{tabular}
    \end{center}
    \caption{Examples of true label non\_informative, predicted label time\_critical}
    \label{tab:qualitative_examples_ni_tc}
\end{table*}

\subsection{Hashtag-heavy tweets}
A lot of tweets labeled as \textbf{support\_and\_relief} by human labelers used a lot of hashtags which makes sense because hashtags provide an easy way of information gathering. However, because we removed hashtags as part of our preprocessing, we ended up losing a lot of context for those tweets which caused DeBerta to label them as \textbf{non\_informative}. Such examples can be found in Table~\ref{tab:qualitative_examples_sr_ni}. The first tweet is correctly labeled as \textbf{support\_and\_relief} by human annotators, but after preprocessing, the tweet just becomes \texttt{looking for} which does not provide any useful information and is therefore classified as \textbf{non\_informative} by DeBerta. However, for tweets that have enough content and were falsely labeled as \textbf{support\_and\_relief} by human annotators, DeBerta successfully classifies them as \textbf{non\_informative}. Therefore, these sorts of misclassifications could be due to our preprocessing rather than an issue with DeBerta.
...
\begin{table*}
    ...
    \begin{tabular}{|p{12cm}|c|}
        ...
        \textbf{Tweet} & \textbf{Category} \\
        \hline
        \#NepalEarthquake Looking for \#CamilaYEzequiel \#Please \#RT \url{https://t.co/akLLoiOmWh} & Crisis-related and actionable by community \\
        \hline
        Lots of backyard work can be time consuming, but linemen do \#WhateverItTakes to get the lights back on! \#Irma… & Crisis related but not actionable \\
        \hline
    \end{tabular}
    \end{center}
    \caption{Examples of true label support\_and\_relief, predicted label non\informative}
    \label{tab:qualitative_examples_sr_ni}
\end{table*}


\subsubsection{Overall Insights}
Overall, the model excels at filtering out non-actionable content, which is highly beneficial in real disaster-response scenarios where minimizing false alarms is essential for effective triage. However, we observed several weaknesses. The model tends to overreact to specific keywords such as “help,” “dead,” “death,” or “explosion”, even when they appear in non-emergency or unrelated contexts. It also struggles with tweets in the \textbf{support\_and\_relief} category because much of their meaning is encoded in hashtags that lose semantic value during preprocessing. To improve performance, future iterations should consider preserving crisis-related hashtags so models can better capture the contextual signals needed for accurate classification.
