...
In the event of a disaster, social media platforms such as Twitter (now known as X) serve as critical channels for real-time information. Time-critical information, such as reports of injured or trapped people, is essential for formal organizations seeking to minimize human casualties. Information about needs for food, water, or medical support is also valuable for both organizations and individuals who aim to provide appropriate assistance to survivors. However, it is challenging and time-consuming to identify such useful information within the flood of messages, as it is mixed with irrelevant or non-actionable content.

The objective of this research is to address this issue by developing a deep learning-based classification system designed to effectively filter and categorize meaningful information within disaster-related tweets. Specifically, we define a multi-class classification task with three categories based on how each tweet can be utilized during disasters. Tweets that convey information needed for handling urgent incidents requiring immediate action (e.g., reports of injured people or infrastructure damage) are labeled as the \textbf{time-critical} class. Tweets containing information useful for assisting rescued or evacuated survivors are labeled as the \textbf{support-and-relief} class. Tweets that do not contain any actionable or relevant information (e.g., a random bystander’s reaction to the disaster) fall under the \textbf{non-informative} class. To this end, multiple deep learning models with different architectures were trained to classify tweets into these three defined categories.
...
Current disaster information gathering often relies on simple keyword and hashtag searching. As pointed out by Wiegmann et al. \cite{Wiegmann2020}, this approach is prone to failure because disaster-related terminology is often diverse and ambiguous. Moreover, many disaster-indicating words also occur in non-disaster contexts, making purely keyword-based filtering unreliable.

Consequently, detecting disaster-related messages is now commonly modeled as a \emph{classification} task tackled with machine learning and, increasingly, deep learning models that can capture richer semantic patterns than simple keyword filters.

% \textit{(5 points) Who cares? If you are successful, what difference will it make?}

This classification system significantly benefit \textbf{emergency responders}, such as 119 operators and on-site rescue teams, by rapidly surfacing critical, actionable intelligence and reducing the chance that urgent calls for help are buried under less relevant content. Ultimately, this can improve situational awareness, increase response efficiency, and potentially reduce casualties.
...
Overall, the learning behavior demonstrates that TextCNN is stable, efficient to train, and capable of capturing the discriminative structure necessary for triage-focused disaster tweet classification.
\begin{figure}
    \centering
