...
TBA: number of parameters to be trained
...
TBA - Vanilla Transformer Text Classifier: Transformer Encoder-based Text Classifier (trained from scratch)
...
% Specifically, a CNN model and a transformer encoder-based text classifier, and fine-tuned a pretrained transformer.
...
As shown in Table \ref{tab:class_distribution}, the dataset is imbalanced. Approximately 65\% of tweets were labeled as non\_informative, while the other two classes each account for only about 18\%. Under such imbalance, accuracy becomes a misleading indicator of performance because a model biased toward the majority class can still achieve a high score. To avoid this, we trained models with macro \textbf{F1-score} as the primary objective, ensuring that minority classes are weighted equally during fine-tuning.
...
After identifying the best-performing model, evaluation will not rely on a single metric. Because each class carries different operational implications during disaster response, we will further analyze false positives and false negatives per class, along with qualitative inspection of misclassified samples. This enables us to assess not only overall effectiveness, but also how reliably the model differentiates urgent information, relief-related reports, and irrelevant content in real-world use.

% We evaluated the performance of three distinct model architectures:
% \begin{enumerate}
%     \item A CNN model utilizing pre-trained GloVe embeddings to assess its ability to extract local n-gram style features.
...
% \subsection{True TC vs. Pred Non\_Informative (358 Total)}

% \textbf{Useless tweets:}
% \begin{itemize}
%     \item how many fertilizer plants are there in texas? \url{http://t.co/es4El50Hdx}
%     \item If ISIS wants a war, I'll bring them a war ὢ4 \url{https://t.co/V7eijo4Flk}
% \end{itemize}

% \textbf{Semi-useful but not actionable by anyone:}
% \begin{itemize}
%     \item RT @severestudios: RT @MattMilletto: \#HurricaneODILE so many crashes and crumbles. Water coming in our 5th floor hotel room. Terrifying. (no mention of location, can’t be seen as a cry for help)
% \end{itemize}

% \textbf{Useful and actionable:}
% \begin{itemize}
%     \item only one actionable item that is a cry for help, but unclear whether it’s related to a crisis: @achohelelwhy: Update - Has Dementia! pls find him \#Missing \#Joplin Charles Writer (\url{http://on.fb.me/kHaGEY}) patient at \#Greenbrier ...
% \end{itemize}

% \textbf{Summary:}
% \begin{itemize}
%     \item deberta is very good at filtering out non-actionable items which aids critical prioritization during crisis response.
%     \item classifies info similar to news-headlines as non\_informative which makes sense in our context:
%     \begin{itemize}
%         \item \#earthquake M 2.9, Puerto Rico region: September 30, 2013 01:18:04 GMT \url{http://t.co/nNh4kBIioG}
%         \item @612brisbane fisher \& camberwell streets, east brisbane. \#bigwet \url{http://t.co/9CANa7p9}
%         \item RT @sunstaronline: [WEATHER] (Updated) Typhoon \#YolandaPH makes 5th landfall over Bantayan Island, Cebu | Sun.Star: \url{http://t.co/0KY3zYswK4}
%         \item \#Earthquake at 50km WNW of Anchor Point, Alaska 2.8 M. Depth 79.6 Km. - 2013-09-27 21:01:48 UTC \url{http://t.co/DsQsXpXJBE}
%     \end{itemize}
% \end{itemize}


% \subsection{True NI vs Pred TC (551 Total)}
% \begin{itemize}
